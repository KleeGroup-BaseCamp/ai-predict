...
class LabelEncoder(FeatureEncoder):
    ...
    def fit(self, data):
        res = self.encoder.fit(data.values.ravel())
        self.params["classes"] = list(res.classes_)
        return self.get_params()

    def transform(self, data):
        return self.encoder.transform(data.values.ravel())
...
class OneHotEncoder(FeatureEncoder):
    ...
    def fit(self, data, other, max_category=None):
        if other:
            if not max_category:
                raise ValueError(
                    "max_category must be filled if other is true")
    ...
    def transform(self, data):
        if self.params["others"]:
            ...
            data[column] = data[column].apply(
                lambda x: x if x in self.params["categories"][0] else "others")
